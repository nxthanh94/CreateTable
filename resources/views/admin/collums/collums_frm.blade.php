                  ...
                  <div class="x_title">
                    <div class="item form-group">
                      ...
                      <div class="col-md-10 col-sm-10 col-xs-12">
                        ...
                        <select id="heard" class="form-control get_table" required name="id_sevice">
                            <option value="0">Chọn dịch vụ</option>
                            @foreach( $service as $key=>$item)
                              <option @if($item['id']==$value_frm['id_sevice']) selected="true" @endif value="{{$item['id']}}">{{$item['name']}}</option>
                            @endforeach
                        </select>
                      </div>
                    </div>
                    <div class="item form-group">
                      <label class="control-label col-md-2 col-sm-2 col-xs-12">Bảng</label>
                      <div class="col-md-10 col-sm-10 col-xs-12">

                        <select id="heard" class="form-control show_table" required name="id_table">
                            <option value="0">Chọn bảng</option>
                            @foreach( $table as $key=>$item)
                              <option @if($item['id']==$value_frm['id_table']) selected="true" @endif value="{{$item['id']}}">{{$item['name']}}</option>
                            @endforeach
                        </select>
                      </div>
                    </div>
...
<div style="display: none;" class="content-frm-collum">
  <div class="item-cot" style="border: 1px solid #ccc; padding: 10px 0px; margin-bottom:20px">
                        <div class="item form-group">
                          ...
                          <div class="col-md-10 col-sm-10 col-xs-12">
                            <input id="name" class="form-control col-md-7 col-xs-12" data-validate-length-range="6" name="name[]" placeholder="Vui lòng nhập tên cột" required="required" type="text" maxlength="255" value="">
                          </div>
                        </div>
                        <div class="item form-group">
                          <label class="control-label col-md-2 col-sm-2 col-xs-12">Type</label>
